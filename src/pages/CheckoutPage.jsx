          ...
          <div className="bg-card/80 backdrop-blur-sm p-6 sm:p-8 rounded-2xl shadow-2xl border border-border/50 mb-8">
            ...
            {cart.length > 0 ? (
              <div className="space-y-3 mb-6">
                {cart.map(item => (
                                    <div key={item.cartId} className="flex justify-between items-start text-sm font-roboto-mono">
                    <div>
                      <span className="text-foreground/90">{item.title} (x{item.quantity})</span>
                      {item.selectedOptions && (
                        <div className="text-xs text-foreground/60 mt-1 pl-2">
                          {Object.entries(item.selectedOptions).map(([key, value]) => value && <div key={key}>+ {key.replace(/_/g, ' ')}</div>)}
                        </div>
                      )}
                    </div>
                    <span className="font-medium text-primary font-minecraft">
                      ${item.totalPrice ? item.totalPrice.toFixed(2) : item.numericPrice.toFixed(2)}
                    </span>
                  </div>
                ))}

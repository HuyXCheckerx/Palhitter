...
const ServiceCard = ({ service, onAddToCart, isHomepageCard = false }) => {
  ...
  if (hasTiers) {
    const allPrices = service.tiers.flatMap(tier => tier.options.map(opt => opt.numericPrice));
    const minPrice = Math.min(...allPrices);
    const maxPrice = Math.max(...allPrices);
    priceDisplay = `$${minPrice} - $${maxPrice}`;
    features = service.tiers[0].features; // Show features from the first tier as a preview
  } else {
    priceDisplay = service.price;
  }

  const isContactForPrice = priceDisplay.toLowerCase().includes('contact');
      ...
      <div className="px-1 pb-1 flex flex-col flex-grow">
        ...
        {features.slice(0, 2).map((feature, idx) => (
            <div key={idx} className="flex items-center space-x-2 mb-1.5">
              ...
              <span className="text-xs text-foreground/60 font-roboto-mono truncate">{feature}</span>
            </div>
          ))}
        ...
        <div className="mt-auto pt-3 flex justify-between items-center">
          ...
          {!isContactForPrice && (
            ...
            >
              <ArrowRight className="mr-1.5" size={14} /> {hasTiers ? 'OPTIONS' : 'VIEW'}
            </Button>
          )}
        </div>
      </div>
